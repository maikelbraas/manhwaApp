    ...
    static async updateSavedManhwa(req, res, next) {
        ...
        for (let manhwa of manhwas) {
            if (manhwa.mid.includes('mgdemon')) {
                ...
                let chapter = content.split('chapter=')[1];
                maxChapter = chapter.split('"')[0];
                console.log(chapter, maxChapter);
                return;
            }
            // else {
    ...
    static async getSavedManhwas(req, res, next) {
        try {
            ...
            for (let manhwa of manhwas) {
                ...
                manhwa.link = manhwa.baseurl + '/title/' + manhwa.slug + "/chapter/" + manhwa.chapter + "/1";
