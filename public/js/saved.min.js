let timer = null; const progressDiv = document.getElementById("progress"); waitTimer(); document.querySelectorAll(".input-number").forEach((x) => (x.autofocus = !1)); document.querySelector('.manhwa-buttons-action').addEventListener('click', async (event) => {
    if (event.target.dataset.button != undefined) {
        const setButtonType = document.querySelector('#' + event.target.dataset.id).dataset; const modalWindow = document.querySelector('#modal-' + event.target.dataset.id)
    ...
    if (event.target.dataset.type != undefined) {
        if (event.target.dataset.type == "remove") { await fetch(`/auth/remove/${event.target.id}`, { method: "DELETE", headers: { "Content-Type": "application/html" } }).then(() => { window.location.reload() }) }
        if (event.target.dataset.type == 'later') { await fetch(`/auth/patch/${event.target.id}`, { method: "PATCH", headers: { "Content-Type": "application/html" } }).then(() => { window.location.reload() }) }
    }
});
