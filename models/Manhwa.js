...
class Manhwa {
    ...
    static async getSavedManhwas(userid) {
        const query = `SELECT manhwas.id, title, mid, slug, media, image, content, chapters, chapter, baseurl, lastUpdate, status FROM manhwas INNER JOIN chaptersSaved ON chaptersSaved.manhwaid = manhwas.mid WHERE userid = ? ORDER BY lastUpdate DESC`;
